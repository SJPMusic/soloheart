...
from shared.models import get_session, Campaign, CampaignSession, Character, ChatMessage, create_database
from cache_manager import cache_manager
from dnd_game.narrative_bridge import NarrativeBridge, DnDMemoryEntry
from narrative_engine.core.character_manager import Race, CharacterClass
from narrative_engine.core.combat_system import Combatant, skill_check_system, combat_system, SkillCheckResult
from narrative_engine.memory.layered_memory import MemoryType, MemoryLayer
...
class EnhancedCampaignManager:
    ...
    def __init__(self, campaign_name: str = "Default Campaign"):
        ...
        self.campaign_data_dir = f"dnd_game/campaigns/{campaign_name}"
        ...
        self.active_quests = []
        self.campaign_settings = self._load_campaign_settings()
    ...
    def _get_or_create_campaign(self, campaign_name: str) -> Campaign:
        ...
        if not campaign:
            campaign = Campaign(
                name=campaign_name,
                world_name=self.campaign_settings.get('world_name', 'fantasy campaign setting'),
    ...
    def start_session(self, session_id: str = None) -> Any:
        ...
        self.current_session = session_summary
    ...
    def end_session(self) -> Any:
        ...
        duration = int((end_time - self.current_session.start_time).total_seconds() / 60)
        ...
        try:
            ...
            for char_record in characters:
                ...
                character = Character(
                    ...
                    ability_scores=char_record.ability_scores
                )
    ...
    def _check_for_skill_check(self, message: str, character: Character = None) -> Optional[SkillCheckResult]:
    ...
    def _character_to_combatant(self, character: Character) -> Combatant:
