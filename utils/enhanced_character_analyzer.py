#!/usr/bin/env python3
"""
Enhanced Character Analyzer - Combines Semantic Understanding with Pattern Matching

This module implements the kind of analysis I do: context-aware, cross-referencing,
and semantic understanding rather than just isolated pattern matching.
"""
...
class EnhancedCharacterAnalyzer:
    ...
    def __init__(self):
        # Context clues for class determination (like I do)
        self.class_context_clues = {
            "fighter": [
    ...
    def extract_full_name(self, text: str) -> str:
        ...
        # Look for full name patterns (like I do)
        full_name_patterns = [
    ...
    def determine_class_with_context(self, text: str) -> str:
        """
        Determine class using context clues and cross-referencing (like I do).
        """
        text_lower = text.lower()
        
        # Score each class based on context clues
        class_scores = {}
        for class_name, clues in self.class_context_clues.items():
            score = 0
            for clue in clues:
    ...
    def extract_trauma_with_context(self, text: str) -> List[str]:
        """
        Extract trauma using semantic understanding (like I do).
        """
        text_lower = text.lower()
        traumas = []
        ...
        return list(set(traumas))
    
    def extract_motivations_with_context(self, text: str) -> List[str]:
        """
        Extract motivations using semantic understanding (like I do).
        """
        text_lower = text.lower()
        motivations = []
    ...
    def extract_gear_with_context(self, text: str) -> List[str]:
        """
        Extract gear using semantic understanding (like I do).
        """
        text_lower = text.lower()
        gear = []
        ...
        gear_patterns = [
            ...
            (r"weapon repair tools", "Weapon repair tools")
        ]
    ...
    def extract_combat_style_with_context(self, text: str) -> str:
        """
        Extract combat style using semantic understanding (like I do).
        """
        text_lower = text.lower()
    ...
    def analyze_all_facts_enhanced(self, text: str) -> Dict[str, Any]:
        """
        Analyze all character facts using enhanced semantic understanding.
        """
        facts = {}
        
        # Use enhanced analysis for each fact type
        facts["name"] = self.extract_full_name(text)
        facts["age"] = extract_age_from_text(text)
        facts["race"] = extract_race_from_text(text)
        facts["class"] = self.determine_class_with_context(text)
        facts["background"] = extract_background_from_text(text)
        facts["gender"] = extract_gender_from_text(text)
        facts["gear"] = self.extract_gear_with_context(text)
        facts["traumas"] = self.extract_trauma_with_context(text)
        facts["motivations"] = self.extract_motivations_with_context(text)
        facts["emotional_themes"] = extract_emotional_themes(text)
        facts["combat_style"] = self.extract_combat_style_with_context(text)
...
def extract_all_facts_enhanced(text: str) -> Dict[str, Any]:
    """
    Extract all character facts using enhanced semantic understanding.
    """
    analyzer = EnhancedCharacterAnalyzer()
    return analyzer.analyze_all_facts_enhanced(text) 