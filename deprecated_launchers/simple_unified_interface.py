...
from llm_entrypoint import chat_completion
...
try:
    ...
    logger.warning("NarrativeEngineProxy not available - memory injection disabled")

# Configure logging
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

# LLM service using pluggable adapter system
...
class SimpleCharacterGenerator:
    ...
    def _real_llm_call(self, messages: List[Dict[str, str]]) -> str:
        """Make a real LLM call using the adapter system."""
        try:
            import time
...
def inject_memory_event(character_name: str, player_input: str, dm_response: str, campaign_id: str):
    """
    Inject a memory event into TNE after a player action using NarrativeEngineProxy.
    ...
    try:
        # Initialize NarrativeEngineProxy
        proxy = NarrativeEngineProxy(campaign_id)
        ...
        # Create memory entry for TNE
        memory_entry = {
            "text": f"{character_name}: {player_input}",
            "metadata": {
        ...
        # Send to TNE via proxy
        def send_async():
            try:
                result = proxy.add_memory(memory_entry)
                if result.get('status') == 'success':
                    logger.info(f"TNE memory injection successful via proxy")
                    
                    # Append to journal after successful TNE injection
                    append_memory_event_to_journal(result, campaign_id)
                else:
                    logger.warning(f"TNE memory injection failed: {result.get('error')}")
