#!/usr/bin/env python3
"""
DnD Web Interface - AI-Powered Solo DnD 5E Game

This interface provides a web-based version of the CLI DnD game,
...
sys.path.append(os.path.join(os.path.dirname(__file__), 'narrative_core'))

# Import DnD game systems
...
def start_character_creation():
    ...
    character_creator.start_creation(creation_type)
...
def get_character_step():
    ...
    return jsonify({
        'step': step.to_dict(),
        'progress': character_creator.get_progress()
    })
...
def make_character_choice():
    ...
    if result.get('completed'):
        ...
        character = character_creator.get_character()
        return jsonify({
            'success': True,
            'completed': True,
...
def save_character():
    ...
    return jsonify({
        'success': True,
        'character': character.to_dict(),
        'message': f'Character {character.name} saved to campaign {current_campaign.name}'
    })
